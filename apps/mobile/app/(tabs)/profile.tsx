...
export default function ProfileScreen() {
  ...
  const [themePreference, setLocalThemePreference] = useState<ThemePreference>('system');
  ...
  useEffect(() => {
    ...
    setLocalThemePreference(mapThemeFromApi(profileQuery.data.theme));
  }, [profileQuery.data]);
          ...
          <View style={styles.sectionStack}>
                ...
                <View style={styles.chipWrap}>
                  {THEME_OPTIONS.map((option) => {
                    ...
                    return (
                      <Pressable
                        key={option.key}
                        onPress={() => setLocalThemePreference(option.key)}
                        style={[
                          styles.chip,
                          {
                      ...
                      >
                        <Text style={[styles.chipText, { color: active ? '#FFFFFF' : colors.textPrimary }]}>{option.label}</Text>
                      </Pressable>
                    );
                  })}
                </View>
              ...
              <View style={styles.preferenceGroup}>
                ...
                <View style={styles.chipWrap}>
                  {(bettingSitesQuery.data ?? []).map((site) => {
                    const active = preferredSites.includes(site.slug);
                    return (
                      <Pressable
                        key={site.id}
                        onPress={() => {
                      ...
                      >
                        <Text style={[styles.chipText, { color: active ? '#FFFFFF' : colors.textPrimary }]}>{site.name}</Text>
                      </Pressable>
                    );
                  })}
                </View>
            ...
            <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
              ...
              {notificationsQuery.data?.items.length ? (
                ...
                <Text style={[styles.preferenceLabel, { color: colors.textSecondary }]}>Ainda não tens notificações.</Text>
              )}
            </View>
