...
- IMPORTANT: Each item has its own "result" field. On Betclic accumulator screenshots each selection row has a small coloured circle icon on the LEFT side of that row: a GREEN circle with a white checkmark (✓) means that selection WON; a RED circle with a white X means that selection LOST; no icon or grey icon means PENDING. CRITICAL: match the icon to the selection in the SAME HORIZONTAL ROW — do not confuse icons from adjacent rows. In a lost accumulator exactly ONE selection will have the red X (the one that caused the loss); all other selections that show a green checkmark must be marked "WON". Do NOT mark a selection LOST just because the overall bet is "Perdida".
