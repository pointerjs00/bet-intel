    ...
    messages: [
      {
        ...
        content: [
          ...
          'On Betclic accumulator screenshots each selection row has a small coloured circle icon on the LEFT side of that row: GREEN circle with white checkmark = WON for that row; RED circle with white X = LOST for that row; no icon or grey = PENDING.',
          'CRITICAL: match each icon to the selection in the SAME HORIZONTAL ROW — do not mis-assign icons from adjacent rows.',
          'In a lost accumulator exactly ONE selection will have the red X (the leg that caused the loss); all other selections that show a green checkmark must be marked WON.',
          'Never mark a selection LOST just because the overall bet is Perdida.',
        ].join(' '),
      },
      {
        role: 'user',
